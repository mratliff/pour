defmodule PourWeb.UserAuthTest do
  use PourWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Pour.Accounts
  alias Pour.Accounts.Scope
  alias PourWeb.UserAuth

  import Pour.AccountsFixtures

  @remember_me_cookie "_pour_web_user_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, PourWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "redirects to lot when user is already logged in", %{conn: conn, user: user} do
      conn = UserAuth.log_in_user(conn, user)
      assert redirected_to(conn) == "/lot"
    end
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      PourWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        PourWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
